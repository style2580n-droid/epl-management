...
LEAGUE_TEAM_MAPS = {
    ...
    # 불필요). 아래 두 리그는 캘린더 연도제(3월/2월~11월/12월)라 8개 리그 중
    # 유일하게 시즌 경계가 다르지만, _find_leagues/_fetch_league_events는
    # 이미 각 리그의 current_season만 보고 도는 방식이라 별도 손질 불필요.
    'mls': {
        '애틀랜타 유나이티드': ['Atlanta United', 'Atlanta United FC'],
        '오스틴 FC': ['Austin FC', 'Austin'],
        'CF 몬트리올': ['CF Montréal', 'CF Montreal'],
        '샬럿 FC': ['Charlotte FC', 'Charlotte'],
        '시카고 파이어': ['Chicago Fire', 'Chicago Fire FC'],
        '콜로라도 래피즈': ['Colorado Rapids'],
        '콜럼버스 크루': ['Columbus Crew', 'Columbus Crew SC'],
        'DC 유나이티드': ['DC United', 'D.C. United'],
        'FC 신시내티': ['FC Cincinnati', 'Cincinnati'],
        'FC 댈러스': ['FC Dallas', 'Dallas'],
        '휴스턴 다이나모': ['Houston Dynamo', 'Houston Dynamo FC'],
        '인터 마이애미': ['Inter Miami', 'Inter Miami CF'],
        'LA 갤럭시': ['LA Galaxy', 'Los Angeles Galaxy'],
        '로스앤젤레스 FC': ['Los Angeles FC', 'LAFC'],
        '미네소타 유나이티드': ['Minnesota United', 'Minnesota United FC'],
        '내슈빌 SC': ['Nashville SC', 'Nashville'],
        '뉴잉글랜드 레볼루션': ['New England Revolution', 'New England'],
        '뉴욕 시티 FC': ['New York City FC', 'NYCFC'],
        '뉴욕 레드불스': ['New York Red Bulls', 'NY Red Bulls'],
        '올랜도 시티': ['Orlando City SC', 'Orlando City'],
        '필라델피아 유니온': ['Philadelphia Union', 'Philadelphia'],
        '포틀랜드 팀버스': ['Portland Timbers'],
        '리얼 솔트레이크': ['Real Salt Lake'],
        '샌디에이고 FC': ['San Diego FC', 'San Diego'],
        '산호세 어스퀘이크스': ['San Jose Earthquakes'],
        '시애틀 사운더스': ['Seattle Sounders FC', 'Seattle Sounders'],
        '스포르팅 캔자스시티': ['Sporting Kansas City', 'Sporting KC'],
        '세인트루이스 시티': ['St. Louis City SC', 'St Louis City', 'St. Louis City'],
        '토론토 FC': ['Toronto FC', 'Toronto'],
        '밴쿠버 화이트캡스': ['Vancouver Whitecaps FC', 'Vancouver Whitecaps'],
    },
    'eliteserien': {
        '올레순': ['Aalesunds FK', 'Aalesund'],
        '보되글림트': ['Bodø/Glimt', 'Bodo/Glimt', 'FK Bodø/Glimt'],
        '프레드릭스타': ['Fredrikstad FK', 'Fredrikstad'],
        '함캄': ['HamKam', 'Hamarkameratene', 'Ham-Kam'],
        'IK 스타트': ['IK Start', 'Start'],
        'KFUM 오슬로': ['KFUM Oslo'],
        '크리스티안순': ['Kristiansund BK', 'Kristiansund'],
        '릴레스트룀': ['Lillestrøm SK', 'Lillestrom SK', 'Lillestrøm'],
        '몰데': ['Molde FK', 'Molde'],
        '로젠보르그': ['Rosenborg BK', 'Rosenborg'],
        '브란': ['SK Brann', 'Brann'],
        '트롬쇠': ['Tromsø IL', 'Tromso IL', 'Tromsø'],
        '발레렝가': ['Vålerenga IF', 'Valerenga IF', 'Vålerenga'],
        '산네피오르': ['Sandefjord Fotball', 'Sandefjord'],
        '사르프스보르그 08': ['Sarpsborg 08', 'Sarpsborg 08 FF', 'Sarpsborg'],
        '비킹': ['Viking FK', 'Viking'],
    },
}
...
def _game_record(stats):
    ...
    return {
        ...
        'shots': int(shots),
        # 2026-07-30 (2-1): EPL판(app_export.py)과 동일 이유로 하드코딩 0 해제.
        # collect_api_football_player_stats.py가 종료경기에 한해 채워 넣는다.
