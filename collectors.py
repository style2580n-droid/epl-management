...
class EloCollector:
    ...
    def run(self, top_n=2000):
        # 2026-07-25 수정: top_n=100(기존 기본값)은 "전세계 통합 Elo 순위" 기준 컷오프라서,
        # 리그 내 순위가 아니라 전세계 순위로 자름 — 그 결과 프리미어리그/라리가/세리에처럼
        # 원래 Elo가 높은 리그의 팀들만 100위 안에 들고, 에레디비시/챔피언십/엘리테세리엔처럼
        # 리그 자체의 평균 Elo가 낮은 곳은 1위 팀조차 전세계 100위 밖으로 밀려나 팀 대부분이
        # ELO 없이 기본값(1500)으로 예측되는 문제가 실전 로그로 확인됨(분데스리가 5/18,
        # 에레디비시 2/18, 챔피언십 5/24, 엘리테세리엔 1/16 — 전부 이 컷오프 때문).
        # ClubElo API 자체는 무인증으로 전세계 수천 클럽을 다 주므로(비용 없음), top_n을 늘려서
        # 8개 리그 전체(최대 약 180팀)가 넉넉히 다 들어오도록 함. 2000이면 사실상 전부 저장됨.
        if not self.elo:
            return
