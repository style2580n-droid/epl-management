...
def load_players(conn, path='data/master/players_pl.json'):
    ...
    for pid, p in players.items():
        conn.execute(
            ...
            '(player_id, name, position, minutes, goals, assists, form, '
            ' status, news, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)',
            (f'fpl:{pid}', p.get('name'), p.get('position'), p.get('minutes'),
             p.get('goals'), p.get('assists'), p.get('form'), p.get('status'),
